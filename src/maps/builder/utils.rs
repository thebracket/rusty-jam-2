use crate::{maps::{TileType, NUM_TILES_X, NUM_TILES_Y, tile_index}, random::Rng};

pub fn decorate_water(tiles: &mut [TileType], rng: &Rng) {
    tiles.iter_mut().filter(|t| **t == TileType::Water).for_each(|t| {
...
pub fn decorate_beach(tiles: &mut [TileType]) {
    let tweaks : Vec<(usize, TileType)> = tiles.iter().enumerate()
        .filter(|(_idx, t)| **t == TileType::CaveFloor)
        .map(|(idx, _)| {
        let (x, y) = ((idx % NUM_TILES_X) as i32, (idx / NUM_TILES_X) as i32);
        let mut bits = 0u8;
        if check(&tiles, x-1, y) { bits += 1 }
        if check(&tiles, x+1, y) { bits += 2 }
        if check(&tiles, x, y-1) { bits += 4 }
        if check(&tiles, x, y+1) { bits += 8 }
...
fn check(tiles: &[TileType], x: i32, y: i32) -> bool {
    x >= 0 && x < NUM_TILES_X as i32 -1 && y >=0 && y < NUM_TILES_Y as i32 -1 && tiles[tile_index(x, y)]!=TileType::CaveFloor
}