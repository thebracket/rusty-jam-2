use bracket_pathfinding::prelude::{BaseMap, Algorithm2D, Point, SmallVec, DijkstraMap};
...
pub fn unreachable(tiles: &[TileType], features: &[TileType], starts: &[usize]) -> Vec<usize> {
    let map = MinimumTileMap {
        tiles: tiles.iter().zip(features.iter()).map(|(t,f)| f.can_player_enter() && t.can_player_enter()).collect()
    };
    let mut dm = DijkstraMap::new(NUM_TILES_X, NUM_TILES_Y, starts, &map, 200.0 );
    DijkstraMap::build(&mut dm, starts, &map);
    dm.map.iter().enumerate().filter(|(_, cost)| **cost > 2000.0).map(|(idx,_)| idx).collect()
}
